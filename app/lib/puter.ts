...
export const usePuterStore = create<PuterStore>((set, get) => {
    ...
    const signIn = async (): Promise<void> => {
        ...
        try {
            // Prefer a redirect-based sign-in so Puter returns to our current URL after auth
            // If the SDK ignores options, it will still work without them
            // @ts-ignore – allow options for broader SDK compatibility
            await puter.auth.signIn({ redirect: true, return_to: typeof window !== "undefined" ? window.location.href : undefined });
    ...
    const init = (): void => {
        ...
        if (puter) {
            set({ puterReady: true });
            // Re-check auth both immediately and whenever the window regains focus
            checkAuthStatus();
            if (typeof window !== "undefined") {
                window.addEventListener("focus", () => {
                    // Only check if Puter is still available
                    if (getPuter()) {
                        checkAuthStatus();
                    }
                });
